...
export default function EngineerJobDetail({ navigation, route }) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'jobs', jobId), (snap) => {
      if (snap.exists()) {
        ...
        setJob(data);
        // Pre-fill note if already submitted once
  ...
  const uploadPhotos = async () => {
    ...
    for (const uri of photos) {
      ...
      const name  = `jobs/${jobId}/${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`;
      const storRef = ref(storage, name);
  ...
  const handleSubmit = async () => {
    if (photos.length === 0) {
      Alert.alert('Photos required', 'Please take at least one photo before submitting.');
      return;
    }
  ...
  const canWork      = isInProgress || isRevision;
            ...
            <>
              ...
              {isDone && (
                <View style={[styles.submittedBox, { backgroundColor: Colors.completedBg }]}>
                  <Text style={[styles.submittedText, { color: Colors.completed }]}>
...
const styles = StyleSheet.create({
  ...
  revisionCard: {
    ...
    borderWidth: 1, borderColor: Colors.needsRevisionBorder,
  },
  ...
  startBtn: {
    ...
    alignItems: 'center', justifyContent: 'center',
    borderBottomWidth: 3, borderBottomColor: '#0F4870',
  },
  submitBtn: {
    backgroundColor: Colors.primary,
    borderRadius: 14, height: 54,
  ...
  submittedBox: {
    backgroundColor: Colors.pendingApprovalBg,
    borderRadius: 12, padding: 16,
    alignItems: 'center',
    borderWidth: 1, borderColor: Colors.pendingApprovalBorder,
  },
